...
def start_service(conns, thread_num, input_que, res_que, plt_que, ctl_que, seed_que):
    ...
    robot = Robot()
    img_id = 1
    class_name = ['diba', 'floor', 'water', 'person', 'robot', 'house']#['broker', 'diba', 'floor', 'water', 'person', 'robot', 'house']
    class_color = [(34, 139, 34),
                    (201, 201, 201),
                    (255, 144, 30),
                    (48, 48, 255),
                    (205, 50, 154),
                    (0, 102, 205)]
    if not ANNO:
        class_color = [(85, 115, 139)] + class_color
        class_name = ['broker'] + class_name
...
def inference_async(pretrained_path, vth, in_que, res_que):
    ...
    # print("vth", vth)
    if ANNO:
        model = mobilenet_slim_spike(15,_if=False, vth=vth)
        cut = 8
    else:
    ...
    while True:
        if in_que.empty():
            ...
            # sleep(1)
            res_que.put((inds, result[:, cut:], img_ids))
